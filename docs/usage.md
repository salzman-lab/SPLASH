# nf-core/stringstats: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/stringstats/usage](https://nf-co.re/stringstats/usage)
...
```console
nextflow run nf-core/stringstats --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```console
nextflow pull nf-core/stringstats
```
...
First, go to the [nf-core/stringstats releases page](https://github.com/nf-core/stringstats/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
