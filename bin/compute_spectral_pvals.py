...
def main():
    ...
    sampleNames, sheetCj = parseSamplesheet(args)

    print('constructing counts dataframe')
    ctsDf = constructCountsDf(df, args)
    ...
    for anch_idx, anch in enumerate(anchLst):
    ...
    if useSheetCj:
        outdf = pd.DataFrame(
            {
                'anchor':anchLst,
                'pval_spectral':pvalsSpectral,
                'pval_rand_init_EM':pvalsRandOpt,
                'pvals_optimized_samplesheetSigned':pvalsSheetSign,
                'pvals_optimized_samplesheet':pvalsSheet
            }
        )
        ...
        outdf = pd.DataFrame(
            {
                'anchor':anchLst,
                'pvalsSpectral':pvalsSpectral,
                'pvalsRandOpt':pvalsRandOpt
            }
        )
    ...
    if args.save_c_f:
        ...
        with open('spectral_cj.npy', 'wb') as f:
            np.save(f,cjArr)

        with open('spectral_f.pkl', 'wb') as handle:
